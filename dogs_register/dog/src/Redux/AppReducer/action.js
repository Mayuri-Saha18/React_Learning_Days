...
export const getApiData = (queryparams) => async (dispatch) => {
  dispatch(getDogsDataRequest());
  try {
    const response = await axios.get("https://masai-fake-jsonserver.onrender.com/dogs", {
      params: queryparams,
    });
    dispatch(getDogsDataSuccess(response.data));
  } catch (error) {
    dispatch(getDogsDataFailure());
  }
};
